...
# lsof -i:8007 2>/dev/null | grep -q LISTEN && echo "Server running on 8007" || (
#   cd /datadrive1/deepak/plan_mall_search/app && 
#   source /datadrive1/deepak/onesearch_venv/bin/activate && 
#   export PYTHONPATH="/datadrive1/deepak/plan_mall_search:/datadrive1/deepak:$PYTHONPATH" && 
#   nohup python3 pro_29_api.py > /tmp/prod_server.log 2>&1 &
...
#    "/datadrive1/deepak/new_mall_pipeline/Dealersearch",
#]
...
import logging
import re
import threading  # OPTIMIZATION: For thread-safe caching
...
# 29062026_v2 index is RED/broken - using 30062026_v2 (fresh index)
PRODUCT_INDEX_NAME = "bajajmall_products_s3_esidx3_29062026_v2"
CATEGORY_INDEX_NAME = "bajajmall_categories_s3_esidx3_29062026_v2"
AUTOSUGGEST_INDEX_NAME = "bajajmall_autosuggest_s3_esidx3_29062026_v2"
BRAND_INDEX_NAME = "bajajmall_brands_s3_esidx3_29062026_v2"  # NEW


#PRODUCT_INDEX_NAME = "bajajmall_products_s3_esidx3_30052026_v2"
#CATEGORY_INDEX_NAME = "bajajmall_categories_s3_esidx3_30052026_v2"
#AUTOSUGGEST_INDEX_NAME = "bajajmall_autosuggest_s3_esidx3_30052026_v2"
#BRAND_INDEX_NAME = "bajajmall_brands_s3_esidx3_30052026_v2"  # NEW

# PRODUCT_INDEX_NAME = "bajajmall_products_s3_esidx3_14052026_v2"
# CATEGORY_INDEX_NAME = "bajajmall_categories_s3_esidx3_14052026_v2"
# AUTOSUGGEST_INDEX_NAME = "bajajmall_autosuggest_s3_esidx3_14052026_v2"
# BRAND_INDEX_NAME = "bajajmall_brands_s3_esidx3_14052026_v2"
...
MAX_REQUEST_TIME_SECONDS = 3.0  # Production limit is 6s, we use 5s for safety margin
ES_TIMEOUT_SECONDS = 3  # Elasticsearch query timeout
FALLBACK_TIMEOUT_SECONDS = 2  # Fallback queries timeout (shorter)

# =================== RESPONSE CACHE (API-LEVEL LATENCY) ===================
# In-memory LRU cache for full API responses - near-instant for cache hits
RESPONSE_CACHE_ENABLED = True
RESPONSE_CACHE_TTL_SECONDS = 3.0  # Short TTL to keep data fresh
RESPONSE_CACHE_MAX_ENTRIES = 256  # Max cached responses

# Shared request executor - avoids per-request thread creation overhead
REQUEST_PARALLEL_EXECUTOR = ThreadPoolExecutor(
    max_workers=4,
    thread_name_prefix="mall_search_parallel"
)

# Response cache storage (thread-safe LRU)
_response_cache: OrderedDict = OrderedDict()
_response_cache_lock = threading.Lock()


def _build_response_cache_key(endpoint: str, payload: Dict) -> str:
    """Build a stable cache key from endpoint + request payload."""
    stable_payload = json.dumps(payload or {}, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
    return f"{endpoint}|{stable_payload}"


def _response_cache_get(cache_key: str) -> Optional[str]:
    """Return cached JSON response body string if present and not expired."""
...
def _response_cache_set(cache_key: str, body_text: str) -> None:
    """Store serialized JSON response body with TTL and LRU eviction."""
    if not RESPONSE_CACHE_ENABLED:
        return
    expires_at = time.time() + RESPONSE_CACHE_TTL_SECONDS
    with _response_cache_lock:
        _response_cache[cache_key] = (expires_at, body_text)
        _response_cache.move_to_end(cache_key)
...
# OPTIMIZATION: ES client with fail-fast settings
es = Elasticsearch(
    "http://localhost:9200",
    request_timeout=5,     # Global timeout to prevent hanging
    max_retries=1,         # Limit retries to reduce latency
    retry_on_timeout=False # Fail fast instead of retrying
)

#def _shutdown_runtime_resources() -> None:
#    """Best-effort resource cleanup at process exit."""
#    try:
#        REQUEST_PARALLEL_EXECUTOR.shutdown(wait=False, cancel_futures=True)
#    except Exception:
#        pass
#    try:
#        es.transport.close()
...
# Load attribute ID to name mapping (using new 09062026 file)
try:
    with open('/data/deepak/new_mall_pipeline_prod/search_codebase_dealesearch_v2/attribute_id_name_mapping_29062026.json', 'r', encoding='utf-8') as f:
...
# OPTIMIZATION: Pre-compute BRAND_NAMES as a set for O(1) lookups instead of O(n) list searches
BRAND_NAMES_SET = set(b.lower() for b in BRAND_NAMES)

# =================== INTENT STOPWORDS ===================
# Natural language intent words that should be stripped from queries
# These are words users use for conversation but don't help search
# e.g., "show me phone" → "phone", "give me laptop" → "laptop"
INTENT_STOPWORDS = {
    # Action/command words
    "show", "give", "find", "get", "search", "look", "looking", "want", "need",
    "display", "list", "tell", "suggest", "recommend", "help", "please",
    # Pronouns and articles
    "me", "my", "i", "a", "an", "the", "some", "any", "this", "that",
    # Question words when used as intents
    "what", "which", "where", "how", "can", "could", "would", "should",
    # Misc intent words
    "for", "to", "of", "with", "like", "best", "good", "top", "latest",
}
...
BIKE_ONLY_MODELS = {
    ...
    "shine", "unicorn", "hornet", "sp125", "livo", "cb350", "cb300", "cb300f",
    "350", "300",  # Shorthand for CB350/CB300
...
SCOOTER_ONLY_MODELS = {
    ...
    # Electric scooters - Ather models
    "ather", "athr", "atheer", "aether",
    "rizta", "rizta s", "rizta z",
    "450s", "450x", "450", "ather 450", "ather 450s", "ather 450x",
...
CANONICAL_MODEL_BRAND_MAP = {
    # ===== BRAND SELF-MAPPINGS (for brand+model detection) =====
    "honda": "honda", "hero": "hero", "tvs": "tvs", "bajaj": "bajaj",
    "yamaha": "yamaha", "suzuki": "suzuki", "ather": "ather", "ola": "ola",
    "royal": "royal enfield", "enfield": "royal enfield", "ktm": "ktm",
    "jawa": "jawa", "yezdi": "yezdi", "aprilia": "aprilia", "vespa": "vespa",
    # ===== HONDA =====
    ...
    "350": "honda",  # Honda CB350 shorthand - common user query
    "cb300": "honda", "cb 300": "honda", "cb300f": "honda",
    "grazia": "honda",
    # ===== HERO =====
    ...
    # ===== ELECTRIC BRANDS =====
    # Ather models (Rizta, 450 series)
    "rizta": "ather", "rizta s": "ather", "rizta z": "ather",
    "450s": "ather", "450 s": "ather", "ather 450s": "ather",
    "450x": "ather", "450 x": "ather", "ather 450x": "ather",
    "450": "ather", "ather 450": "ather",
    "ather": "ather", "athr": "ather", "aether": "ather", "atheer": "ather",
...
MOBILE_PHONE_KEYWORDS = {
    ...
    "galaxy", "flip phone", "foldable phone","note", "fold", "flip", "s23", "s24", "s25", "a54", "a34", "a55", "a35",
...
# Coffee appliance keywords (must NOT be mapped to furniture coffee table intent)
COFFEE_APPLIANCE_KEYWORDS = {
    "coffee machine", "coffe machine", "coffee maker", "coffe maker",
    "coffee maker machine", "coffe maker machine", "coffee brewer", "coffee makers",
    "espresso machine", "espresso maker", "cappuccino machine", "barista machine",
    "bean to cup", "drip coffee", "coffee espresso", "coffee appliance"
}
...
# Atta Chakki / Flour Mill keywords (including typos and variations)
ATTA_CHAKKI_KEYWORDS = {
    # Standard terms
    "atta chakki", "attachakki", "atta chakki machine", "atta chakki machines",
    "flour mill", "flourmill", "flour mills", "flour grinder", "flourgrinder",
    "atta machine", "atta maker", "flour maker", "grain mill", "grain grinder",
    "ata chakki", "ata chaki", "aata chakki", "aata chaki",
    
    # Common typos and variations
    "atta chaki", "atta caki", "atta ckai", "atta chkki", "atta chaki",
    "ata chaki", "ata caki", "ata chakki", "atat chakki", "atat chaki",
    "atta chakii", "atta chakkii", "atta chkii", "atta chaaki",
    "attachaki", "atachaaki", "atachakki", "atachaki",
    "flor mill", "flour mil", "flour mll", "flur mill", "flour mill",
    "flour griner", "flour ginder", "flour grinder",
    "gehu chakki", "gehun chakki", "wheat grinder", "wheat mill",
    
    # Hindi/regional
    "chakki", "chaki", "chaaki", "chkki", "chakkin", "chakin"
}
...
WASHING_MACHINE_KEYWORDS = {
    ...
    # Short forms and slang - NOTE: "w m" removed to prevent false positives (e.g., "show me" contains "w m")
    "wm", "w/m",
...
REALME_KEYWORDS = {
    ...
    # Hindi-style phonetic typos
    "riyalmi", "riyelmi", "riyalme", "riyelme", "ryalmi", "ryalme",
...
APPLE_PRODUCTS_MAP = {
    ...
    "mac air": "laptops",
    "macair": "laptops",
    # MacBook with inch sizes (Apple doesn't make 12 inch, normalize to closest)
    "macbook 12 inch": "laptops",
    "macbook 13 inch": "laptops",
    "macbook 14 inch": "laptops",
    "macbook 15 inch": "laptops",
    "macbook 16 inch": "laptops",
    "macbook pro 12 inch": "laptops",
    "macbook pro 13 inch": "laptops",
    "macbook pro 14 inch": "laptops",
    "macbook pro 15 inch": "laptops",
    "macbook pro 16 inch": "laptops",
    "macbook air 13 inch": "laptops",
    "macbook air 15 inch": "laptops",
    # MacBook with chips
    "macbook m1": "laptops",
    "macbook m2": "laptops",
    "macbook m3": "laptops",
...
def is_out_of_scope_query(query: str) -> Tuple[bool, Optional[str]]:
    ...
    WHITELIST_PATTERNS = {
        ...
        # Flour mill / Atta chakki machine (contain "flour"/"wheat" but are NOT groceries)
        "flour mill", "flour mills", "flourmill", "flour grinder", "flourgrinder",
        "flor mill", "flour mil", "flur mill",
        "wheat grinder", "wheat mill", "grain mill", "grain grinder",
        "atta chakki", "attachakki", "ata chakki", "aata chakki",
        "atta machine", "chakki machine", "gehu chakki", "gehun chakki",
    }
...
BRAND_TYPO_CORRECTIONS = {
    ...
    # Hindi-style phonetic typos
    "riyalmi": "Realme", "riyelmi": "Realme", "riyalme": "Realme", "riyelme": "Realme",
    "ryalmi": "Realme", "ryalme": "Realme", "rialme": "Realme", "rialmi": "Realme",
...
# OPTIMIZATION: Pre-computed protected words set for typo correction (avoids recreation on every call)
_PROTECTED_WORDS = frozenset({
    "phone", "phones", "mobile", "mobiles", "watch", "watches", 
    ...
    # HAIR CARE WORDS - Prevent "hair" → "haier" fuzzy brand matching
    "hair", "dryer", "drier", "dryers", "driers", "straightener",
    "curler", "styler", "hairdryer", "blowdryer", "airwrap"
})
...
class QueryProcessor:
    ...
    def __init__(self):
        ...
        # OPTIMIZATION: Pre-compile regex patterns for typo correction (avoids recompilation on every request)
        self._precompile_typo_fixes()
        # OPTIMIZATION: Cache brand_list for fuzzy matching (avoid set->list conversion on every word)
        self._brand_list_cached = list(self.brand_set)
        # OPTIMIZATION: Pre-compile frequently used regex patterns in detect_category
        self._RE_COFFEE = re.compile(r'\bcoffe?e?\b')
        self._RE_PHONE_MOBILE = re.compile(r'\b(phone|phones|mobile|mobiles)\b')
        # OPTIMIZATION: Pre-compute category word sets and synonym lowercased forms
        self._category_word_sets = {cat_key: set(cat_key.lower().split()) for cat_key in CATEGORY_CANONICAL.keys()}
        self._synonym_lower_map = []
        for cat_key, synonyms in self.synonym_map.items():
            for syn in synonyms:
                self._synonym_lower_map.append((syn.lower(), cat_key))
        # OPTIMIZATION: Pre-compile short mobile keywords regex patterns (avoid re.compile per keyword per request)
        self._short_mobile_kw_patterns = [
            (kw, re.compile(r'\b' + re.escape(kw) + r'\b'))
            for kw in MOBILE_PHONE_KEYWORDS if len(kw) <= 3
        ]
        self._long_mobile_kw_set = frozenset(kw for kw in MOBILE_PHONE_KEYWORDS if len(kw) > 3)
        # OPTIMIZATION: Pre-compile category name regex patterns
        self._category_regex_patterns = [
            (cat_key.lower(), re.compile(r'\b' + re.escape(cat_key.lower()) + r'\b'), canonical)
            for cat_key, canonical in CATEGORY_CANONICAL.items()
        ]
        # OPTIMIZATION: Pre-compile synonym regex patterns
        self._synonym_regex_patterns = []
        for syn_lower, cat_key in self._synonym_lower_map:
            self._synonym_regex_patterns.append((syn_lower, re.compile(r'\b' + re.escape(syn_lower) + r'\b'), cat_key))
        # OPTIMIZATION: Pre-compute full_categories for fuzzy matching (avoid list comprehension per request)
        self._full_categories = [k for k in CATEGORY_CANONICAL.keys() if len(k) > 3]
    ...
    def _precompile_typo_fixes(self):
        """Pre-compile all regex patterns for typo correction - called once at init"""
        brand_typo_fixes_raw = [
            # Copy the EXACT same list from correct_typos method - all the (pattern, replacement) tuples
            ...
            # Hindi-phonetic Realme typos (riyalmi, riyelmi, etc.)
            (r'\briyalmi\b', 'realme'), (r'\briyelmi\b', 'realme'), (r'\briyalme\b', 'realme'),
            (r'\briyelme\b', 'realme'), (r'\bryalmi\b', 'realme'), (r'\bryalme\b', 'realme'),
            (r'\brialmi\b', 'realme'), (r'\briealme\b', 'realme'), (r'\brielme\b', 'realme'),
        ...
        # OPTIMIZATION: Pre-compile all patterns once with IGNORECASE flag
        self._BRAND_TYPO_FIXES_COMPILED = [(re.compile(pattern, re.IGNORECASE), replacement) for pattern, replacement in brand_typo_fixes_raw]
    ...
    def correct_typos(self, query: str) -> str:
        ...
        # OPTIMIZATION: removed local `import re` (already imported at module level line 21)
        ...
        # OPTIMIZATION: Use pre-compiled regex patterns from __init__ instead of recompiling on every call
        # Common brand typos - fix at start of word or standalone
        # Using lookahead to handle compound words like "poccom6" -> "pocom6"
        # (Patterns are pre-compiled in _precompile_typo_fixes() method)
        for compiled_pattern, replacement in self._BRAND_TYPO_FIXES_COMPILED:
            query_lower = compiled_pattern.sub(replacement, query_lower)
        ...
        # OPTIMIZATION: Use module-level _PROTECTED_WORDS frozenset instead of creating a new set on every call
        protected_words = _PROTECTED_WORDS
        ...
        for word in words:
            ...
            brand_match = fuzzproc.extractOne(word, self._brand_list_cached, scorer=fuzz.ratio, score_cutoff=85)
            if brand_match:
    ...
    def detect_category(self, query: str) -> Optional[str]:
        ...
        # Highest priority: coffee appliance intent (including typo: coffe)
        # Must run early to avoid conflicts like "espresso" (car model S-Presso).
        has_coffee_token = bool(self._RE_COFFEE.search(query_lower))
        has_coffee_appliance_word = any(
            kw in query_lower for kw in [
                "machine", "maker", "espresso", "cappuccino", "barista", "brewer"
            ]
        )
        has_espresso_style_intent = any(kw in query_lower for kw in ["espresso", "cappuccino", "barista"]) and any(
            kw in query_lower for kw in ["machine", "maker", "brewer"]
        )
        has_furniture_table_context = "table" in query_words or "tables" in query_words
        if (has_coffee_token and has_coffee_appliance_word and not has_furniture_table_context) or \
           (has_espresso_style_intent and not has_furniture_table_context):
        ...
        # =================== HIGHEST PRIORITY: CAMERA & ACCESSORIES DETECTION ===================
        # MUST come BEFORE two-wheeler detection to prevent "camera accessories" → "access" scooter
        # But do NOT hijack "camera phone" queries away from smartphone intent.
        _phone_context_keywords = ["phone", "phones", "mobile", "mobiles", "smartphone", "smartphones", "5g"]
        has_phone_context = any(pk in query_lower for pk in _phone_context_keywords)
        if has_phone_context and ("camera phone" in query_lower or "camera" in query_lower):
            return "smartphone"

        _camera_keywords = [
            "camera", "cameras", "dslr", "webcam", "action camera",
            "camera accessories", "camera accessory", "camera & accessories",
            "camera and accessories", "camera lens", "camera bag", "tripod",
            # Typos
            "camra", "cammera", "camara", "camare", "camrea"
        ]
        for camkw in _camera_keywords:
            if camkw in query_lower:
                return "camera accessories"
        
        # =================== HIGHEST PRIORITY: HAIR DRYER / HAIR CARE DETECTION ===================
        # MUST come BEFORE brand fuzzy matching to prevent "hair" → "Haier"
        _hair_dryer_keywords = [
            "hair dryer", "hairdryer", "hair drier", "hairdrier",
            "hair dryers", "hairdryers", "hair driers", "hairdriers",
            "blow dryer", "blowdryer", "blow drier", "blowdrier",
            "hair straightener", "straightener", "hair curler", "curling iron",
            "hair styler", "hair care", "haircare",
            # Dyson specific
            "dyson airwrap", "airwrap", "supersonic", "dyson supersonic",
            # Typos
            "hiar dryer", "hair dryr", "hair dryar", "hair driar",
            "har dryer", "hare dryer", "haire dryer"
        ]
        for hdkw in _hair_dryer_keywords:
            if hdkw in query_lower:
                return "health and personal care appliance"
        # Also check if "hair" appears with "dryer/drier" in query words
        if ("hair" in query_words or "hiar" in query_words or "haire" in query_words) and \
           any(dw in query_words for dw in ["dryer", "drier", "dryers", "driers", "dryr"]):
            return "health and personal care appliance"
        
        # =================== HIGHEST PRIORITY: FLIP/FOLDABLE PHONE DETECTION ===================
        # MUST come BEFORE generic smartphone detection
        _foldable_phone_keywords = [
            "flip phone", "flipphone", "flip phones", "foldable phone", "foldablephone",
            "foldable phones", "fold phone", "folding phone", "folding phones",
            "samsung fold", "samsung flip", "galaxy fold", "galaxy flip",
            "z fold", "zfold", "z flip", "zflip", "galaxy z",
            "motorola razr", "moto razr", "razr",
            # Typos
            "flipfone", "foldphone", "filp phone", "flp phone"
        ]
        for fpkw in _foldable_phone_keywords:
            if fpkw in query_lower:
                return "smartphone"
        
        # =================== HIGHEST PRIORITY: BRAVIA TV DETECTION ===================
        # "bravia", "bravia tv", "bravia led" should boost Sony TV
        if "bravia" in query_lower:
            return "television"
        
        # =================== HIGHEST PRIORITY: ENGINE CC DETECTION ===================
        # "120 cc", "150cc", "cc automatic" etc. indicate two-wheeler queries
        # This MUST come BEFORE washing machine detection (since "automatic" is also a WM keyword)
        import re as _re
        _cc_pattern = _re.compile(r'\b(\d{2,4})\s*cc\b', _re.IGNORECASE)
        if _cc_pattern.search(query_lower) or "cc" in query_words:
            # If "cc" is present with or without a number, it's a two-wheeler query
            # Also handle "automatic" in this context as transmission type, not washing machine type
            return "two wheeler"
        
        # =================== HIGHEST PRIORITY: PULSAR → BAJAJ BIKE DETECTION ===================
        # "pulsar" and typos should explicitly return two wheeler (Bajaj)
        _pulsar_keywords = [
            "pulsar", "pulser", "plsar", "plser", "pulsr", "pulsaar", 
            "pulsur", "pulzar", "pulsarr", "pulsser", "pluser", "plusar",
            "bajaj pulsar", "pulsar 150", "pulsar 125", "pulsar 200", "pulsar 220",
            "pulsar ns", "pulsar rs", "pulsar n160", "pulsar n250"
        ]
        for plkw in _pulsar_keywords:
            if plkw in query_lower:
                return "two wheeler"
        ...
        # IMPORTANT: Skip if "dish" is in query to avoid "dishwasher" → "washer" substring match
        ...
        # Handle price-based car queries like "cars under 10 lakhs", "car below 5 lakh"
        _car_price_patterns = [
            r'cars?\s+under\s+\d+\s*(?:lakh|lac|lakhs|lacs)',
            r'cars?\s+below\s+\d+\s*(?:lakh|lac|lakhs|lacs)',
            r'cars?\s+within\s+\d+\s*(?:lakh|lac|lakhs|lacs)',
            r'cars?\s+upto\s+\d+\s*(?:lakh|lac|lakhs|lacs)',
            r'cars?\s+up\s+to\s+\d+\s*(?:lakh|lac|lakhs|lacs)',
            r'(?:budget|cheap|affordable)\s+cars?',
            r'cars?\s+(?:for|in)\s+\d+\s*(?:lakh|lac|lakhs|lacs)',
        ]
        for pattern in _car_price_patterns:
            if re.search(pattern, query_lower):
                return "car"
        ...
        # =================== TEMPLE / MANDIR DETECTION (FURNITURE) ===================
        # "temple", "mandir", "pooja mandir" etc. are furniture items (religious/puja cabinets)
        # MUST detect to avoid matching "Tempur" bed brand
        temple_mandir_keywords = [
            "temple", "temples", "mandir", "mandirs", "pooja mandir", "puja mandir",
            "home temple", "wooden temple", "pooja temple", "puja temple",
            "tempal", "tempul", "tample", "mandr", "mndir", "mnadir"
        ]
        for keyword in temple_mandir_keywords:
            if keyword in query_lower:
                return "furniture"
        ...
        # =================== HIGH PRIORITY: VISI COOLER (display/visual cooler) ===================
        # Specific product type "visi cooler" should NOT be classified as generic air cooler
        # Match exact phrase or compacted form (e.g., "visicooler")
        if "visi cooler" in query_lower or "visicooler" in query_no_space:
            return "visi cooler"
        ...
        # Priority 0.252: Coffee machine/maker intent (including common typo: coffe)
        # Must be before furniture/atta-chakki fallbacks.
        for keyword in COFFEE_APPLIANCE_KEYWORDS:
            if keyword in query_lower:
                return "kitchen appliances"
        
        # Priority 0.254: Check for ATTA CHAKKI / FLOUR MILL keywords
        for keyword in ATTA_CHAKKI_KEYWORDS:
            if keyword in query_lower or keyword in query_words:
                return "atta chakki machine"
        # Fuzzy match for atta chakki typos
        atta_fuzzy_exclusions = {
            "chair", "chairs", "table", "tables", "sofa", "sofas", "bed", "beds",
            "office", "outdoor", "recliner", "cabinet", "cabinets", "wardrobe", "bookshelf"
        }
        for word in query_words:
            if len(word) >= 4:
                if word in atta_fuzzy_exclusions:
                    continue
                # Check for chakki/chaki variations
                # Guard: require chak/chakki-like shape to avoid false positives like "chair" -> "chaki"
                if ("chak" in word or "chki" in word or "ckki" in word) and (
                    fuzz.ratio(word, "chakki") >= 75 or fuzz.ratio(word, "chaki") >= 75
                ):
                    return "atta chakki machine"
                # Check for atta variations
                if fuzz.ratio(word, "atta") >= 80 and len(query_words) > 1:
                    # Only match "atta" if there's another word (to avoid false positives)
                    for other_word in query_words:
                        if other_word != word and (fuzz.ratio(other_word, "chakki") >= 70 or fuzz.ratio(other_word, "chaki") >= 70 or fuzz.ratio(other_word, "machine") >= 75):
                            return "atta chakki machine"
        # Check for flour mill fuzzy match
        if "flour" in query_lower or "flor" in query_lower or "flur" in query_lower:
            for word in query_words:
                if fuzz.ratio(word, "mill") >= 75 or fuzz.ratio(word, "grinder") >= 75:
                    return "atta chakki machine"
        # Check compound without space
        if "attachakki" in query_no_space or "atachaaki" in query_no_space or "flourmill" in query_no_space:
            return "atta chakki machine"
        ...
        # IMPORTANT: Skip words containing "dish" to avoid "dishwasher" → "washer" fuzzy match
        for word in query_words:
            if len(word) >= 6 and "dish" not in word:
        ...
        # OPTIMIZATION: Use pre-compiled short keyword regex patterns and pre-computed long keyword set
        for kw, pattern in self._short_mobile_kw_patterns:
            if pattern.search(query_lower):
                return "smartphone"
        for keyword in self._long_mobile_kw_set:
            if keyword in query_lower:
                return "smartphone"
        
        # Priority 0.6: "phone" or "phones" query -> smartphone category (all brands)
        if self._RE_PHONE_MOBILE.search(query_lower):
            return "smartphone"
        
        # Priority 1: Exact word match for category names
        # OPTIMIZATION: Use pre-computed category word sets and regex patterns
        for cat_key, cat_word_set in self._category_word_sets.items():
            canonical = CATEGORY_CANONICAL[cat_key]
            # Check if all category words are present in query
            if cat_word_set.issubset(query_words):
                return canonical
        for cat_key_lower, cat_regex, canonical in self._category_regex_patterns:
            if cat_regex.search(query_lower):
                return canonical
        
        # Priority 2: Check synonym matches (exact word boundary)
        # OPTIMIZATION: Use pre-compiled synonym regex patterns
        for syn_lower, syn_regex, cat_key in self._synonym_regex_patterns:
            if syn_lower in query_words or syn_regex.search(query_lower):
        ...
        # OPTIMIZATION: Use pre-computed full_categories list
        match = fuzzproc.extractOne(query_lower, self._full_categories, scorer=fuzz.token_set_ratio, score_cutoff=85)
        if match:
    ...
    def detect_brand(self, query: str) -> Optional[str]:
        ...
        # Priority -1: Check for Bravia → Sony (Sony's TV line)
        if "bravia" in query_lower:
            return "sony"
        ...
        # =================== TWO-WHEELER MODEL → BRAND RESOLUTION ===================
        # If query contains a known two-wheeler model name, resolve the brand from CANONICAL_MODEL_BRAND_MAP
        # e.g., "pulsar" → brand "bajaj", "activa" → brand "honda"
        TWO_WHEELER_BRAND_SET = {"bajaj", "honda", "hero", "tvs", "yamaha", "suzuki", "royal enfield",
                                  "ather", "ola", "okinawa", "ampere", "bgauss", "revolt", "pure ev"}
        TWO_WHEELER_CONTEXT_WORDS = {"bike", "bikes", "scooter", "scooters", "two wheeler", "2 wheeler"}
        for word in query_words:
            if word in CANONICAL_MODEL_BRAND_MAP:
                mapped_brand = CANONICAL_MODEL_BRAND_MAP[word]
                if mapped_brand in TWO_WHEELER_BRAND_SET:
                    return mapped_brand
        ...
        # Hair care context words - skip "haier" match when query is about hair care
        _hair_care_words = {"dryer", "drier", "dryers", "driers", "straightener", "curler", 
                            "styler", "airwrap", "supersonic", "blow", "blowdryer", "hairdryer"}
        _has_hair_care_context = bool(query_words & _hair_care_words)
        
        for word in query_words:
            if len(word) >= 3:  # Only try fuzzy on words with 3+ chars
                # Skip fuzzy matching "hair" to "haier" when hair care context is present
                if word == "hair" and _has_hair_care_context:
                    continue
                ...
                if brand:
                    ...
                    if brand.lower() == "tvs" and (any(w in query_words for w in _tv_context_words) or
                                                    re.search(r'\b\d{2,3}\s*tvs?\b', query_lower)):
                        continue
                    # Skip "haier" brand if hair care context is present
                    if brand.lower() == "haier" and _has_hair_care_context:
                        continue
    ...
    def detect_product_boost(self, query: str) -> Dict[str, Any]:
        ...
        # PRIORITY: Coffee appliance intent should stay in kitchen appliances
        # This avoids "coffee machine" being misrouted to furniture coffee tables.
        for keyword in COFFEE_APPLIANCE_KEYWORDS:
            if keyword in query_lower:
                return {
                    "boost_type": "coffee_machine",
                    "boost_terms": [
                        "coffee machine", "coffee maker", "espresso machine", "cappuccino machine"
                    ],
        ...
        if "coffee table" in query_lower or "center table" in query_lower:
            return {
    ...
    def process(self, query: str) -> Dict[str, Any]:
        ...
        if is_apple:
            ...
            flag_keywords_to_preclean = [
                ...
                # Non-product context words (showroom, dealer, store, etc.)
                "showroom", "showrooms", "dealer", "dealers", "dealership",
                "store", "stores", "shop", "shops", "outlet", "outlets",
                "near me", "nearby", "in my city", "in my area"
            ]
            ...
            # =================== STRIP INTENT STOPWORDS ===================
            # Remove natural language intent words (show, me, give, etc.) that don't help search
            # But keep meaningful product words like brand names, models, categories
            # e.g., "show me phone" → "phone", "give me samsung" → "samsung"
            if processed:
                processed_words = processed.split()
                # Keep words that are NOT just intent stopwords
                # But preserve the query if ALL words would be removed
                filtered_words = [w for w in processed_words if w.lower() not in INTENT_STOPWORDS]
                if filtered_words:
                    processed = " ".join(filtered_words)
                # else: keep original processed if all words would be stripped
            ...
            if category == "two wheeler":
                ...
                # IMPORTANT: Prefer actual MODEL names over BRAND names
                # A model name is one that maps to a DIFFERENT brand (e.g., "450s" → "ather")
                # A brand name maps to itself (e.g., "ather" → "ather")
                specific_model_found = None
                brand_word_found = None
                for word in processed_words:
                    if word in SPECIFIC_MODEL_NAMES:
                        mapped = CANONICAL_MODEL_BRAND_MAP.get(word, "")
                        if mapped.lower() != word.lower():
                            # This is a MODEL name (maps to different brand)
                            specific_model_found = word
                            break
                        else:
                            # This is a BRAND name (maps to itself)
                            brand_word_found = word
                
                # If no model found but brand was found, use brand
                if not specific_model_found and brand_word_found:
                    # Check if there are other words that could be model numbers
                    # e.g., "ather 450s" - "450s" should be kept even if "ather" was matched first
                    for word in processed_words:
                        if word != brand_word_found and word in SPECIFIC_MODEL_NAMES:
                            specific_model_found = word
                            break
                ...
                if specific_model_found:
                    # Specific model found (pulsar, bullet, activa, 450s, rizta, etc.)
                    # EXPAND shorthand model numbers to full names for better ES matching
                    # e.g., "350" → "cb350" when brand is Honda
                    MODEL_EXPANSIONS = {
                        "honda": {"350": "cb350", "300": "cb300"},
                        "bajaj": {"200": "pulsar", "150": "pulsar"},
                    }
                    if brand_word_found and brand_word_found in MODEL_EXPANSIONS:
                        expansions = MODEL_EXPANSIONS[brand_word_found]
                        if specific_model_found in expansions:
                            expanded_model = expansions[specific_model_found]
                            logger.info(f"Expanding model '{specific_model_found}' to '{expanded_model}' for brand '{brand_word_found}'")
                            specific_model_found = expanded_model
                    
                    # KEEP search text as brand + model for better text matching
                    # e.g., "ather 450s" should search for "ather 450s" not just "450s"
                    if brand_word_found:
                        processed = f"{brand_word_found} {specific_model_found}"
                    else:
                        processed = specific_model_found
            ...
            # Also set electric flag when brand is an electric scooter brand
            # This ensures Ather, Ola, etc. products (asset_category_name="Electric") are not filtered by "Scooters" filter
            ELECTRIC_SCOOTER_BRANDS = {"ather", "ola", "okinawa", "ampere", "bgauss", "revolt", "pure ev", "battre", "ivoomi"}
            if brand and brand.lower() in ELECTRIC_SCOOTER_BRANDS:
                is_electric_scooter_query = True
                logger.info(f"Electric scooter brand '{brand}' detected - setting is_electric_scooter_query=True")
        ...
        CATEGORY_ONLY_KEYWORDS = {
            ...
            # Flour mill / Atta chakki → search for "atta chakki" (products are named atta chakki, not flour mill)
            "flour mill": "atta chakki", "flour mills": "atta chakki", "flourmill": "atta chakki",
            "flour grinder": "atta chakki", "flourgrinder": "atta chakki",
            "flor mill": "atta chakki", "flour mil": "atta chakki", "flur mill": "atta chakki",
            "wheat grinder": "atta chakki", "wheat mill": "atta chakki",
            "grain mill": "atta chakki", "grain grinder": "atta chakki",
            "gehu chakki": "atta chakki", "gehun chakki": "atta chakki",
            # Flip/Foldable phone → search for "samsung galaxy z fold flip" to find those products
            "flip phone": "samsung galaxy z fold flip", "flipphone": "samsung galaxy z fold flip",
            "foldable phone": "samsung galaxy z fold flip", "foldablephone": "samsung galaxy z fold flip",
            "fold phone": "samsung galaxy z fold flip", "folding phone": "samsung galaxy z fold flip",
            # Temple / Mandir → search for "pooja mandir" (avoid matching Tempur bed brand)
            "temple": "pooja mandir", "temples": "pooja mandir",
            "home temple": "pooja mandir", "wooden temple": "pooja mandir",
            "mandir": "pooja mandir", "mandirs": "pooja mandir",
            "pooja mandir": "pooja mandir", "puja mandir": "pooja mandir",
            "pooja temple": "pooja mandir", "puja temple": "pooja mandir",
            # Temple typos
            "tempal": "pooja mandir", "tempul": "pooja mandir", "tample": "pooja mandir",
            "mandr": "pooja mandir", "mndir": "pooja mandir", "mnadir": "pooja mandir",
        }
        
        processed_lower = processed.lower().strip()
        
        # Handle price-based car queries: "cars under X lakhs" → "car"
        if re.search(r'cars?\s+(under|below|within|upto|up\s+to)\s+\d+\s*(lakh|lac|lakhs|lacs)', processed_lower):
            processed = "car"
            logger.info(f"Price-based car query normalized → search text 'car'")
        elif processed_lower in CATEGORY_ONLY_KEYWORDS and category:
...
class SearchEngine:
    ...
    def __init__(self, es_client, index_name: str):
        ...
        # OPTIMIZATION: Pre-computed keyword tuples for hand-crafted fixes (avoid list creation per request)
        self._CHETAK_KEYWORDS = ("chetak", "chetk", "chetek", "chatak", "cheetek", "chetaak")
        self._PULSAR_KEYWORDS = ("pulsar", "pulser", "plsar", "plser", "pulsr", "pulsaar", "pulsur", "pulzar")
        # OPTIMIZATION: Pre-computed non-relevance sorts as frozenset (avoid tuple creation per request)
        self._NON_RELEVANCE_SORTS = frozenset((
            "low_to_high", "lowtohigh", "price_low_high", "emi_low_high", "asc",
        ...
        # OPTIMIZATION: Pre-compiled regex patterns for model number detection
        self._MODEL_NUMBER_RE = re.compile(r'\d')
        self._NETWORK_PREFIX_RE = re.compile(r'\b5g\b|\b4g\b|\b3g\b')
    ...
    def build_query(self, query_info: Dict, city_id: str = None, filters: Dict = None,
        ...
        if bike_only_filter and category == "two wheeler":
            # Filter to show only bikes/motorcycles, exclude scooters AND electric
            # asset_category_name in ES: "Mid range premium bikes", "High range premium bikes", 
            # "Super Bikes", "Executive", "Economy", "Mopeds"
            # Exclude: "Scooters", "Electric" (electric scooters like E Access, Ather, Ola)
            # NOTE: User searching "brand bike" wants petrol bikes, not e-scooters
            filter_clauses.append({
                "bool": {
                    "must_not": [
            ...
            logger.info("Bike-only filter applied - excluding Scooters and Electric")
        ...
        if attributes.get("storage") or attributes.get("storage_tb"):
            ...
            # Standard storage values in ES index: 16, 32, 64, 128, 256, 512 GB; 1, 2 TB
            # For non-standard values (e.g., 200GB), map to nearest HIGHER standard value
            # This handles queries like "mobile 200gb" → show 256GB phones
            STANDARD_GB_VALUES = [8, 16, 32, 64, 128, 256, 512]
            
            if storage_tb:
                storage_value = str(storage_tb) + " TB"  # Format as "1 TB", "2 TB"
            elif storage_gb in STANDARD_GB_VALUES:
                # Exact standard value - use as is
                storage_value = str(storage_gb) + " GB"
            else:
                # Non-standard value - find nearest higher standard value
                # e.g., 200 → 256, 100 → 128, 300 → 512
                nearest_higher = None
                for std_val in sorted(STANDARD_GB_VALUES):
                    if std_val >= storage_gb:
                        nearest_higher = std_val
                        break
                
                if nearest_higher:
                    storage_value = str(nearest_higher) + " GB"
                    logger.info(f"Non-standard storage {storage_gb}GB mapped to nearest: {storage_value}")
                else:
                    # Value is higher than all GB values, try TB
                    storage_value = "1 TB"
                    logger.info(f"Non-standard storage {storage_gb}GB mapped to: {storage_value}")
        ...
        # Price filter on mrp field (Maximum Retail Price at product level)
        # MRP is a top-level field, not nested under city_offers
        if filters and ('price_max' in filters or 'price_range' in filters):
            ...
            if price_range:
                # Build simple range query on mrp field
                mrp_range_query = {"range": {"mrp": {}}}
                if "gte" in price_range:
                    mrp_range_query["range"]["mrp"]["gte"] = price_range["gte"]
                if "lte" in price_range:
                    mrp_range_query["range"]["mrp"]["lte"] = price_range["lte"]
                
                filter_clauses.append(mrp_range_query)
                logger.info(f"Added MRP price filter: {price_range}")
    ...
    def get_global_emi_slider_range(self, query_info: Dict, city_id: str = None, 
        ...
        try:
            ...
            response = self.es.search(index=self.index, body=es_query, request_timeout=3, request_cache=True)
    ...
    def search(self, query_info: Dict, city_id: str = None, filters: Dict = None,
        ...
        try:
            ...
            # OPTIMIZATION: Use pre-computed tuple from __init__
            for kw in self._CHETAK_KEYWORDS:
            ...
            # OPTIMIZATION: Use pre-computed tuple from __init__
            for kw in self._PULSAR_KEYWORDS:
                if kw in original_lower:
                    query_info["category"] = "two wheeler"
            ...
            _has_model_number = bool(self._MODEL_NUMBER_RE.search(self._NETWORK_PREFIX_RE.sub('', _processed)))
            ...
            if _is_multi_word_query:
                ...
                logger.debug(f"Multi-word query: Over-fetching {page_size} hits for exact phrase reranking (original: {original_page_size})")
            ...
            sort_by_lower = (sort_by or "").lower().strip()
            # OPTIMIZATION: Use pre-computed frozenset from __init__ instead of creating tuple on every request
            non_relevance_sorts = self._NON_RELEVANCE_SORTS
            ...
            if total == 0 and sort_by_lower in non_relevance_sorts:
                ...
                fallback_response = self.es.search(index=self.index, body=fallback_es_query, request_timeout=2, request_cache=True)
            ...
            if total == 0 and city_id and city_id not in ['citi_id_0', '0', 'citi_id_']:
                ...
                global_fallback_response = self.es.search(index=self.index, body=global_fallback_es_query, request_timeout=2, request_cache=True)
            ...
            if total == 0 and query_info.get("detect_new_launch"):
                ...
                nl_fallback_resp = self.es.search(index=self.index, body=nl_fallback_query, request_cache=True)
            ...
            if total > 0 and is_relevance_sort and page == 1 and not _is_model_query and not _has_flag_filter:
                ...
                if needed_nl > 0:
                    ...
                    if filters and 'price_range' in filters:
                        price_range = filters.get('price_range', {})
                        if price_range:
                            # Build MRP price filter for new_launch fetch
                            nl_price_filter = {"range": {"mrp": {}}}
                            if "gte" in price_range:
                                nl_price_filter["range"]["mrp"]["gte"] = price_range["gte"]
                            if "lte" in price_range:
                                nl_price_filter["range"]["mrp"]["lte"] = price_range["lte"]
                            nl_base_filter.append(nl_price_filter)
                            logger.info(f"BRD: Added MRP price filter to new_launch fetch: {price_range}")
            ...
            # =================== APPLE PRODUCT SIZE FALLBACK ===================
            # When "MacBook 12 inch" returns 0 results (no 12" MacBooks exist), 
            # show MacBooks without the specific size
            if total == 0 and query_info.get("is_apple_query"):
                _processed = query_info.get("processed", "").lower()
                _attributes = query_info.get("attributes", {})
                # Check if query contains screen size that yielded 0 results
                _has_inch_size = re.search(r'\b(\d{1,2})\s*(inch|inches|"|in)\b', _processed) or _attributes.get("screen_size")
                if _has_inch_size:
                    logger.info(f"Apple query with screen size returned 0, trying without size...")
                    fallback_info = query_info.copy()
                    # Remove inch size from processed query
                    _new_processed = re.sub(r'\b\d{1,2}\s*(inch|inches|"|in)\b', '', _processed, flags=re.IGNORECASE).strip()
                    _new_processed = re.sub(r'\s+', ' ', _new_processed).strip()
                    fallback_info["processed"] = _new_processed if _new_processed else "macbook"
                    # CRITICAL: Also clear the screen_size attribute to prevent filter from being applied
                    fallback_attrs = fallback_info.get("attributes", {}).copy()
                    if "screen_size" in fallback_attrs:
                        del fallback_attrs["screen_size"]
                    fallback_info["attributes"] = fallback_attrs
                    fallback_query = self.build_query(fallback_info, city_id, filters, page, page_size, emi_range, sort_by)
                    response = self.es.search(index=self.index, body=fallback_query, request_timeout=2)
                    total = response["hits"]["total"]["value"]
                    logger.info(f"Apple size fallback returned {total} results")
            
            # =================== CAR PRICE RANGE FALLBACK ===================
            # When "cars under 10 lakhs" returns 0 (no cars in that price range),
            # show all cars sorted by price (cheapest first)
            if total == 0 and query_info.get("category") == "car":
                _original = query_info.get("original", "").lower()
                _has_price_range = re.search(r'(under|below|within|upto|up\s*to|less\s*than)\s*\d+\s*(lakh|lac|lakhs|lacs|k|thousand|cr|crore)', _original)
                if _has_price_range:
                    logger.info(f"Car price range query returned 0, showing all cars...")
                    fallback_info = query_info.copy()
                    fallback_info["processed"] = "car"  # Simple search
                    # Don't apply price filter
                    fallback_query = self.build_query(fallback_info, city_id, filters, page, page_size, emi_range, "low_to_high")
                    response = self.es.search(index=self.index, body=fallback_query, request_timeout=2)
                    total = response["hits"]["total"]["value"]
                    logger.info(f"Car price fallback returned {total} results (sorted by price)")
            
            # =================== TWO-WHEELER BRAND FALLBACK (EARLY CHECK) ===================
            # When "pulsar" returns bikes from wrong brand (or 0 results),
            # explicitly search for bikes/motorcycles from all brands
            # This is an EARLY check to ensure two-wheeler model queries show bikes
            _tw_original = query_info.get("original", "").lower()
            _tw_brand = query_info.get("brand", "").lower() if query_info.get("brand") else ""
            _tw_category = query_info.get("category", "")
            
            # Two-wheeler models that should show bikes (typically Bajaj models not in data)
            _bajaj_bike_models = {"pulsar", "pulser", "plsar", "dominar", "avenger", "platina", "ct100", "ct110", "ct125"}
            _bajaj_scooter_models = {"chetak", "chetk", "chetek"}
            
            # Check if this is a Bajaj model query (Bajaj not in our two-wheeler data)
            _is_bajaj_bike_query = any(m in _tw_original for m in _bajaj_bike_models)
            _is_bajaj_scooter_query = any(m in _tw_original for m in _bajaj_scooter_models)
            
            if (_is_bajaj_bike_query or _is_bajaj_scooter_query) and _tw_category == "two wheeler":
                # Check if results are from wrong brand or 0 results
                _should_fallback_tw = False
                if total == 0:
                    _should_fallback_tw = True
                elif total > 0:
                    # Check if results are from Bajaj - if not, we're already showing fallback
                    _has_bajaj = False
                    for hit in response["hits"]["hits"][:5]:
                        _brand_in_result = hit.get("_source", {}).get("manufacturer_desc", "").lower()
                        if "bajaj" in _brand_in_result:
                            _has_bajaj = True
                            break
                    # If no Bajaj in results but user searched Bajaj model, override with explicit bike search
                    if not _has_bajaj:
                        _should_fallback_tw = True
                        logger.info(f"Two-wheeler query '{_tw_original}' returned non-Bajaj results, showing all bikes...")
                
                if _should_fallback_tw:
                    fallback_info = query_info.copy()
                    fallback_info["brand"] = None  # Remove brand filter
                    
                    if _is_bajaj_scooter_query:
                        fallback_info["processed"] = "scooter"
                        logger.info(f"Bajaj scooter model not in data, showing all scooters...")
                    else:
                        fallback_info["processed"] = "motorcycle bike"
                        logger.info(f"Bajaj bike model not in data, showing all bikes...")
                    
                    fallback_query = self.build_query(fallback_info, city_id, filters, page, page_size, emi_range, sort_by)
                    response = self.es.search(index=self.index, body=fallback_query, request_timeout=2)
                    total = response["hits"]["total"]["value"]
                    logger.info(f"Two-wheeler brand fallback returned {total} results")
...
class ResponseFormatter:
    ...
    def format_response(cls, search_result: Dict, city_id: str = None, 
        ...
        # Show what filters were parsed from the query (EMI/price/attributes)
        applied_filters = {}
        if query_info:
            ...
            if parsed_filters:
                ...
                if "mrp" in parsed_filters:
                    price_filter = parsed_filters["mrp"]
                    applied_filters["price"] = {
                        "min": price_filter.get("gte"),
                        "max": price_filter.get("lte")
                    }
            
            # Add attribute filters parsed from query
            attributes = query_info.get("attributes", {})
            if attributes:
                attr_filters = {}
                # Smart storage mapping - show what was actually searched
                if attributes.get("storage") or attributes.get("storage_tb"):
                    storage_gb = attributes.get("storage", 0)
                    storage_tb = attributes.get("storage_tb")
                    STANDARD_GB_VALUES = [8, 16, 32, 64, 128, 256, 512]
                    ...
                    if storage_tb:
                        attr_filters["storage"] = {"parsed": str(storage_tb) + " TB", "applied": str(storage_tb) + " TB"}
                    elif storage_gb in STANDARD_GB_VALUES:
                        attr_filters["storage"] = {"parsed": str(storage_gb) + " GB", "applied": str(storage_gb) + " GB"}
                    else:
                        # Non-standard value - find nearest higher
                        nearest_higher = None
                        for std_val in sorted(STANDARD_GB_VALUES):
                            if std_val >= storage_gb:
                                nearest_higher = std_val
                                break
                        if nearest_higher:
                            attr_filters["storage"] = {"parsed": str(storage_gb) + " GB", "applied": str(nearest_higher) + " GB"}
                        else:
                            attr_filters["storage"] = {"parsed": str(storage_gb) + " GB", "applied": "1 TB"}
                
                if attributes.get("ram"):
                    attr_filters["ram"] = str(attributes["ram"]) + " GB"
                if attributes.get("capacity_kg"):
                    attr_filters["capacity_kg"] = str(attributes["capacity_kg"]) + " kg"
                if attributes.get("capacity_litres"):
                    attr_filters["capacity_litres"] = str(attributes["capacity_litres"]) + " L"
                if attributes.get("capacity_litres_range"):
                    attr_filters["capacity_litres"] = attributes["capacity_litres_range"]
                if attributes.get("tonnage"):
                    attr_filters["tonnage"] = str(attributes["tonnage"]) + " Ton"
                if attributes.get("screen_size"):
                    attr_filters["screen_size"] = str(attributes["screen_size"]) + " inch"
                if attributes.get("star_rating"):
                    attr_filters["star_rating"] = str(attributes["star_rating"]) + " Star"
                if attributes.get("load_type"):
                    attr_filters["load_type"] = attributes["load_type"]
                if attributes.get("door_type"):
                    attr_filters["door_type"] = attributes["door_type"]
                if attributes.get("ac_type"):
                    attr_filters["ac_type"] = attributes["ac_type"]
                if attributes.get("color"):
                    attr_filters["color"] = attributes["color"]
                
                if attr_filters:
                    applied_filters["attributes"] = attr_filters
...
def mall_search_api():
    ...
    try:
        ...
        # Check cache first for near-instant response (< 1ms for hits)
        cache_key = _build_response_cache_key("mall_search", data)
        cached_response = _response_cache_get(cache_key)
        if cached_response:
            logger.info(f"CACHE HIT: mall_search in {(time.time() - start_time)*1000:.1f}ms")
        ...
        # CITY PRIORITY: User's city products appear first, but show complete catalog
        raw_city_id = data.get('city_id', data.get('cityId', data.get('cityid', '0')))
        
        # Normalize city_id to "citi_id_XXX" format for consistent processing
        if raw_city_id and raw_city_id not in ('0', '', 'citi_id_0'):
            # Extract number from various formats
            if raw_city_id.startswith('citi_id_'):
                city_id = raw_city_id  # Already in correct format
            else:
                # Just a number like "504" or "21"
                city_id = f'citi_id_{raw_city_id}'
            logger.info(f"City priority enabled: {city_id} (user's city products will appear first)")
        else:
            # No specific city - use global catalog
            city_id = 'citi_id_0'
            logger.info(f"Global search - no city priority (raw_city_id: {raw_city_id})")
        ...
        if price_emi_filters:
            ...
            if 'mrp' in price_emi_filters:
                # Price filter from query
                filters['price_range'] = price_emi_filters['mrp']
        ...
        query_info["enhanced_parsing"] = {
            ...
            "comparison_field": "lowest_emi" if price_emi_filters and "lowest_emi" in price_emi_filters else "mrp"
        }
        ...
        if page == 1 and is_dealer_search_enabled() and dealer_search_type != 'product':
            try:
                import sys
                print(f"DEBUG: Starting dealer intent classification for '{query}'", file=sys.stderr, flush=True)
                ...
                if intent_result:
                    ...
                    print(f"DEBUG: Intent: {intent_result.intent.name}, confidence: {intent_result.confidence:.2f}", file=sys.stderr, flush=True)
                    ...
                    if dealer_search_type == 'dealer':
                        ...
                        # Also includes two-wheeler brands (Honda, TVS, etc.) and EV brands (Ather, Ola)
                        # that have both products AND showrooms/dealers
                        _CONFLICT_BRAND_NAMES = {
                            # Electronics retailers
                            "croma", "chroma", "kroma", "croma store", "chroma store",
                            "reliance digital", "reliance",
                            # Two-wheeler brands (have showrooms AND products)
                            "ather", "aether", "atheer", "ola", "ola electric",
                            "honda", "tvs", "hero", "bajaj", "suzuki", "yamaha",
                            "royal enfield", "ktm", "kawasaki", "bmw", "harley",
                            "okinawa", "ampere", "bgauss", "revolt", "pure ev",
                        }
                        ...
                        if _matched_conflict_brand:
                            ...
                            try:
                                # DEBUG: Log conflict brand match
                                print(f"DEBUG: Conflict brand matched: {_matched_conflict_brand}", flush=True)
                                print(f"DEBUG: query_info is_electric: {query_info.get('is_electric_scooter_query')}", flush=True)
                                print(f"DEBUG: query_info brand: {query_info.get('brand')}", flush=True)
                                print(f"DEBUG: query_info category: {query_info.get('category')}", flush=True)
                                ...
                                print(f"DEBUG: Conflict product count: {product_count}", flush=True)
        ...
        response_json = json.dumps(unified_response, ensure_ascii=False, separators=(',', ':'))
        _response_cache_set(cache_key, response_json)
...
# from new_mall_pipeline.autosuggest.autosuggest_v2 import init_autosuggest, get_autosuggest_engine
...
def mall_autosuggest_api():
    """
    Autosuggest API - Returns max 7 suggestions with dealer cards
    ...
    Response Format:
        {
            ...
            "response": {
                ...
                "query_type": "brand",
                "timing_ms": 1.82,
                "Card": [{"dealer": [{"name": "Croma", "adress": "...", "dealer_id": "22"}]}]
            }
        }
    """
    try:
        ...
        # =================== RESPONSE CACHE CHECK ===================
        cache_key = _build_response_cache_key("autosuggest", data)
        cached_response = _response_cache_get(cache_key)
        if cached_response:
            logger.debug(f"CACHE HIT: autosuggest in {(time.time() - start_time)*1000:.1f}ms")
            return app.response_class(
                response=cached_response,
                status=200,
                mimetype='application/json'
            ...
            try:
                ...
                if dealer_cards or dealer_keywords:
                    ...
                    for dc in dealer_cards:
                        ...
                        formatted_dealer_cards.append({
                            "name": dc.get("name", ""),
                            "adress": dc.get("address", ""),  # Note: 'adress' per API spec (intentional)
                            "dealer_id": str(dealer_id),
                            "distance_in_km": dc.get("distance_in_km"),
                            "redirection_Url": dc.get("redirection_Url", "")
                        })
                    ...
                    logger.info(
                        f"Dealer autosuggest: {len(formatted_dealer_cards)} cards, "
                        f"{len(dealer_keywords)} keywords merged for '{query}'"
                        f"{' (prefix-match)' if _is_dealer_prefix else ''}"
                    )
            except Exception as de:
        ...
        # =================== BUILD FINAL RESPONSE ===================
        # Transform internal result format to required API format with data.autoSuggest wrapper
        inner_response = result.get("response", {})
        ...
        if is_known_dealer and formatted_dealers:
            # For dealer-specific queries like "Croma", show "Croma stores near you"
        ...
        final_response = {
            ...
            "response": {
                ...
                "chips": inner_response.get("chips", []),
                "filter_text": filter_text,
                "chip_query_prefix": inner_response.get("chip_query_prefix", ""),
                "synonym_suggestions": inner_response.get("synonym_suggestions", []),
                "language": inner_response.get("language", "english"),
                "query_type": inner_response.get("query_type", ""),
                "timing_ms": round(elapsed, 2),
                "Card": card_section
            }
        }
        
        logger.info(f"Autosuggest API: '{query}' -> {len(inner_response.get('keywords', []))} suggestions, {len(formatted_dealers)} dealers in {elapsed:.2f}ms")
        
        # =================== CACHE RESPONSE ===================
        response_json = json.dumps(final_response, ensure_ascii=False, separators=(',', ':'))
        _response_cache_set(cache_key, response_json)
...
if __name__ == '__main__':
    ...
    get_autosuggest()
    #app.run(host="0.0.0.0", port=8564, debug=False, threaded=True)
