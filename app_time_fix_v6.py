...
#PRODUCT_INDEX_NAME = "bajajmall_products_s3_esidx3_25052026_v2"
#CATEGORY_INDEX_NAME = "bajajmall_categories_s3_esidx3_25052026_v2"
#AUTOSUGGEST_INDEX_NAME = "bajajmall_autosuggest_s3_esidx3_25052026_v2"
#BRAND_INDEX_NAME = "bajajmall_brands_s3_esidx3_25052026_v2"
...
es = Elasticsearch(
    "http://localhost:9200",
    request_timeout=ES_TIMEOUT_SECONDS,
    max_retries=1,
...
# Load attribute ID to name mapping (using new 05052026 file)
try:
    with open('/data/deepak/new_mall_pipeline_prod/search_codebase_dealesearch_v2/idx/attribute_id_name_mapping_05052026.json', 'r', encoding='utf-8') as f:
...
class TTLCache:
    ...
    def __init__(self, maxsize=2000, ttl_seconds=300):
        self._cache = {}
    ...
    def set(self, key, value):
        with self._lock:
            if len(self._cache) >= self._maxsize:
                keys = sorted(self._cache.keys(), key=lambda k: self._cache[k]['ts'])
                for k in keys[:self._maxsize // 4]:
                    del self._cache[k]
...
query_cache = TTLCache(maxsize=2000, ttl_seconds=300)
...
# =================== UNIFIED KEYWORD → CATEGORY LOOKUP ===================
# Single dict lookup instead of checking 15+ sets sequentially
KEYWORD_TO_CATEGORY_HINT = {}

for _kw in AUDIO_VIDEO_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "audio video"
for _kw in AIR_FRYER_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "air fryer"
for _kw in GEYSER_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "water heater"
for _kw in SOLAR_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "solar"
for _kw in SOLAR_HEATER_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "solar heater"
for _kw in MIXER_GRINDER_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "mixer grinder"
for _kw in TWO_WHEELER_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "two wheeler"
for _kw in FOUR_WHEELER_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "four wheeler"
for _kw in MOBILE_PHONE_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "smartphones"
for _kw in LAPTOP_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "laptops"
for _kw in WATCH_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "watch and wearable"
for _kw in DISHWASHER_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "dishwasher"
for _kw in WASHING_MACHINE_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "washing machine"
for _kw in ATTA_CHAKKI_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "atta chakki"
for _kw in ONEPLUS_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "smartphones"
for _kw in REALME_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "smartphones"
for _kw in REDMI_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "smartphones"
for _kw in IQOO_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "smartphones"
for _kw in GOOGLE_PIXEL_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "smartphones"
for _kw in KITCHEN_KEYWORDS:
    KEYWORD_TO_CATEGORY_HINT[_kw] = "kitchen appliance"
for _kw in TWO_WHEELER_MODELS:
    KEYWORD_TO_CATEGORY_HINT.setdefault(_kw, "two wheeler")
for _kw in BIKE_ONLY_MODELS:
    KEYWORD_TO_CATEGORY_HINT.setdefault(_kw, "motorcycle")
for _kw in SCOOTER_ONLY_MODELS:
    KEYWORD_TO_CATEGORY_HINT.setdefault(_kw, "scooter")

del _kw  # cleanup loop variable

# =================== CACHED FUZZY MATCH ===================
_fuzzy_brand_cache = {}
_fuzzy_brand_strict_cache = {}

def cached_fuzzy_match_brand(query_lower):
    """Cached fuzzy brand matching - same input never recomputed"""
    if query_lower in _fuzzy_brand_cache:
        return _fuzzy_brand_cache[query_lower]
    
    if len(_fuzzy_brand_cache) > 5000:
        _fuzzy_brand_cache.clear()
    
    result = fuzzproc.extract_one(
        query_lower,
        BRAND_NAMES,
        scorer=fuzz.ratio,
        score_cutoff=75
    )
    _fuzzy_brand_cache[query_lower] = result
    return result

def cached_fuzzy_match_brand_strict(word, brand_list):
    """Cached fuzzy brand matching with strict cutoff (85) for typo correction"""
    _key = word
    if _key in _fuzzy_brand_strict_cache:
        return _fuzzy_brand_strict_cache[_key]
    
    if len(_fuzzy_brand_strict_cache) > 5000:
        _fuzzy_brand_strict_cache.clear()
    
    result = fuzzproc.extractOne(word, brand_list, scorer=fuzz.ratio, score_cutoff=85)
    _fuzzy_brand_strict_cache[_key] = result
    return result

# =================== PARALLEL ES QUERY EXECUTION ===================
_es_executor = ThreadPoolExecutor(max_workers=6)

def execute_es_queries_parallel(queries_dict):
    """
    Execute multiple ES queries in parallel.
    
    queries_dict: {
        "label1": {"index": "idx_name", "body": query_body},
        "label2": {"index": "idx_name", "body": query_body},
    }
    Returns: {"label1": result1, "label2": result2}
    Failed queries return None (don't block others).
    """
    results = {}
    futures = {}
    
    for label, params in queries_dict.items():
        future = _es_executor.submit(
            es.search,
            index=params["index"],
            body=params["body"]
        )
    ...
    for future in as_completed(futures, timeout=MAX_REQUEST_TIME_SECONDS):
        label = futures[future]
        try:
            results[label] = future.result(timeout=0.5)
        except Exception as e:
            logger.error(f"Parallel ES query failed [{label}]: {e}")
            results[label] = None
    
    # Fill missing labels with None
    for label in queries_dict:
        if label not in results:
            results[label] = None
            logger.warning(f"Parallel ES query timed out [{label}]")
    
    return results

# =================== ES MULTI-SEARCH HELPER ===================
def execute_msearch(queries_list):
    """
    Execute multiple ES queries in a single HTTP request.
    queries_list: [("index_name", query_body), ...]
    Returns: [result1, result2, ...]
    """
    body = []
    for index_name, query_body in queries_list:
        body.append({"index": index_name})
        body.append(query_body)
    try:
        response = es.msearch(body=body, request_timeout=ES_TIMEOUT_SECONDS)
        return response.get('responses', [None] * len(queries_list))
    except Exception as e:
        logger.error(f"Multi-search failed: {e}")
        return [None] * len(queries_list)
...
class QueryProcessor:
    ...
    def correct_typos(self, query: str) -> str:
        ...
        for word in words:
            ...
            # Try fuzzy match against brands (cached for performance)
            brand_match = cached_fuzzy_match_brand_strict(word, list(self.brand_set))
            if brand_match:
    ...
    def detect_category(self, query: str) -> Optional[str]:
        ...
        # =================== FAST PATH: Unified keyword lookup ===================
        # Quick check using KEYWORD_TO_CATEGORY_HINT for early exit on common queries
        # This replaces iterating through 15+ keyword sets sequentially
        # Note: High-priority special cases (washing machine, TV, etc.) are checked below
        _fast_hint = KEYWORD_TO_CATEGORY_HINT.get(query_lower)
        if _fast_hint:
            # Map the hint to the actual return value used in priority chain
            _hint_map = {
                "audio video": "audio video",
                "air fryer": "home appliance",
                "water heater": "water heater and geysers",
                "solar": "inverter",
                "solar heater": "water heater",
                "mixer grinder": "kitchen appliances",
                "two wheeler": "two wheeler",
                "four wheeler": "car",
                "smartphones": "smartphone",
                "laptops": "laptops",
                "watch and wearable": "watch and wearable",
            ...
            _mapped = _hint_map.get(_fast_hint, _fast_hint)
            logger.debug(f"Fast category hint: '{query_lower}' → '{_fast_hint}' → '{_mapped}'")
            return _mapped
        
        # Also check individual query words against the lookup
        for _qw in query_words:
            _word_hint = KEYWORD_TO_CATEGORY_HINT.get(_qw)
            if _word_hint:
                _hint_map = {
                    "audio video": "audio video",
                    "air fryer": "home appliance",
                ...
                _mapped = _hint_map.get(_word_hint, _word_hint)
                logger.debug(f"Fast word hint: '{_qw}' → '{_word_hint}' → '{_mapped}'")
                return _mapped
        ...
        for word in query_words:
            if len(word) >= 4:
                ...
                if fuzz.ratio(word, "chakki") >= 75 or fuzz.ratio(word, "chaki") >= 75:
                    return "atta chakki machine"
    ...
    def build_query(self, query_info: Dict, city_id: str = None, filters: Dict = None,
        ...
        es_query = {
            ...
            "_source": {
                "includes": [
                    # Core identity
                    "modelid", "model_id", "model_launch_date",
                    # Display
                    "product_name", "image", "sku", "product_url", "keyword",
                    "manufacturer_id", "manufacturer_desc",
                    # Classification
                    "actual_category", "category_type",
                    "asset_category_id", "asset_category_name",
                    # Flags & ratings
                    "mkp_active_flag", "avg_rating", "rating_count", "mop",
                    # SKU attributes
                    "attribute_set_id", "attribute_swatch_color",
                    # Dynamic attribute_* fields (wildcard - needed for filters)
                    "attribute_*",
                    # Nested objects
                    "products", "city_offers",
                    # Script sort / precision scoring fields
                    "search_field", "sku_name",
                ]
            },
                ...
                sku_fallback_query = {
                    ...
                    "_source": {
                        "includes": [
                            ...
                            "asset_category_id", "asset_category_name",
                            "mkp_active_flag", "avg_rating", "rating_count", "mop",
                            "attribute_set_id", "attribute_swatch_color", "attribute_*",
                            "products", "city_offers", "search_field", "sku_name",
                        ]
                    },
                    "collapse": {
                        "field": "modelid",
...
def mall_search_api():
    ...
    try:
        ...
        cache_key = hashlib.md5(
            f"{query}|{json.dumps(filters or {}, sort_keys=True)}".encode()
        ).hexdigest()
        ...
        # =================== GLOBAL EMI SLIDER CALCULATION + SEARCH (PARALLEL) ===================
        # Run EMI slider calculation and main search in parallel for latency improvement
        # OPTIMIZATION: Skip EMI slider for pages > 1 (user already has slider range from page 1)
        if page > 1:
            ...
            # Page 1: Run EMI slider + main search IN PARALLEL
            elapsed = time.time() - start_time
            _emi_future = None
            _search_future = None
            
            with ThreadPoolExecutor(max_workers=2) as _parallel_exec:
                # Submit EMI slider calculation (only if time budget allows)
                if elapsed < (MAX_REQUEST_TIME_SECONDS - 2.0):
                    _emi_future = _parallel_exec.submit(
                        search_engine.get_global_emi_slider_range,
                        query_info=query_info,
                        city_id=city_id,
                        filters=filters if filters else None
                    )
                ...
                # Submit main search
                _search_future = _parallel_exec.submit(
                ...
                # Collect EMI slider result
                if _emi_future is not None:
                    try:
                        emi_slider_range = _emi_future.result(timeout=3.0)
                        logger.info(f"Global EMI slider range: {emi_slider_range}")
                    except Exception as e:
                        logger.error(f"EMI slider calculation failed: {e}")
                        emi_slider_range = {"min": 0, "max": 0}
                else:
                    emi_slider_range = {"min": 0, "max": 0}
                ...
                try:
                    search_result = _search_future.result(timeout=5.0)
                except Exception as e:
                    logger.error(f"Main search failed: {e}")
                    search_result = {"success": False, "hits": [], "total": 0, "aggregations": {}}
        ...
        # === CACHE STORE ===
        query_cache.set(cache_key, unified_response)
...
def latency_check():
    """Diagnostic endpoint to measure latency improvements"""
    times = {}
    
    # Test ES connection
    t0 = time.time()
    es.ping()
    times['es_ping'] = f"{(time.time()-t0)*1000:.1f}ms"
    
    # Test ES query
    t0 = time.time()
    es.search(index=PRODUCT_INDEX_NAME, body={"size": 1, "query": {"match_all": {}}})
    times['es_query_1doc'] = f"{(time.time()-t0)*1000:.1f}ms"
    
    # Test cache
    query_cache.set("_test_key", {"test": True})
    t0 = time.time()
    query_cache.get("_test_key")
    times['cache_get'] = f"{(time.time()-t0)*1000:.3f}ms"
    
    times['cache_size'] = len(query_cache._cache)
